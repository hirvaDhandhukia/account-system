...
// reset password submit btn dabaya hoo to hi ye run hoga warna error dee dega
    if (isset($_POST['reset-password-submit'])) {
        ...
        if(empty($password) || empty($passwordRepeat)) {
            header("location: ..\create-new-password-admin.php?newpwd=empty&selector=" . $selector . "&validator=" . $validator);
            echo "empty passwords not allowed";
            exit();
        } else if ($password !== $passwordRepeat) {
            header("location: ..\create-new-password-admin.php?newpwd=pwdnotsame&selector=" . $selector . "&validator=" . $validator);
        ...
        require "../../users/includes/config.php";
                        ...
                        if(!$row = mysqli_fetch_assoc($result)) {
                            ...
                            if(!mysqli_stmt_prepare($stmt, $sql)) {
                                ...
                                if(!mysqli_stmt_prepare($stmt, $sql)) {
                                    ...
                                    mysqli_stmt_bind_param($stmt, "s", $tokenEmail);
                                    mysqli_stmt_execute($stmt);
                                    header("location: ..\login-admin.php?newpwd=passwordupdated");
                                }
